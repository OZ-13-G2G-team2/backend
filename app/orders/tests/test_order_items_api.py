...
class OrderItemsAPITest(APITestCase):
    def setUp(self):
        ...
        self.product = Product.objects.create(name="티셔츠", price=12000, stock=10)
    ...
    def test_get_order_items(self):
        response = self.client.get(f"/order_items/?order_id={self.order.id}")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)

    def test_update_order_item_quantity(self):
        response = self.client.patch(
            f"/order_items/{self.order_item.id}/",
    ...
    def test_delete_order_item(self):
        response = self.client.delete(f"/order_items/{self.order_item.id}/")
        self.assertIn(
