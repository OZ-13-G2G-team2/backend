...
class OrderItemsAPITest(APITestCase):
    ...
    def setUp(self):
        ...
        self.order = Order.objects.create(user=self.user, user_address='서울시 강남구', payment_method='card')
    ...
    def test_update_order_item_quantity(self):
        response = self.client.patch(f'/order-items/{self.order_item.id}/', {'quantity': 3, 'change_reason': '테스트'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
